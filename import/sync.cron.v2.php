...
// Функции
function logWrite($str, $direction='')
{
	global $fLog;
	global $fLastLog;
...
$fLastLog = fopen($path."last_log.htm", "w");
$locked = flock($fLastLog, LOCK_EX | LOCK_NB);
if(!$locked) {
    echo 'Не удалось получить блокировку';
    exit(-1);
}
...
<pre>
';
fwrite($fLastLog, $template_start);
...
$filetime = filemtime($file_input);
$curtime = time();
$timetemplate="d-m-Y H:i:s";
$actuality=($curtime-$filetime)/60;

logWrite("Время файла экспорта ".date($timetemplate, $filetime));
//logWrite("Текущее время	".date($timetemplate, $curtime));
logWrite("Актуальность файла выгрузки из 1С на момент операции импорта: ".round($actuality)." мин.");
if($actuality>120){
	logWrite('<span style="color:red; font-weight:bold;">Файл устарел, необходимо звонить IT-шникам!</span>');
	exit(1);
}
